...
- `images_app.py`：统一部署入口。定义三个模型 cls（ZImageModel / DBZ8Model / Flux2Klein9B），各自挂不同的 image / volume / secret。
...
- 语法检查：`python -m py_compile images_api.py images_app.py`
...
- `model` 为必填，取值见 `GET /v1/models`（`z-image-turbo` / `dbz8-sda` / `flux.2-klein-9b`）。
